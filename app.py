...
from ui.components import apply_theme, render_tool_status, render_memory_status, render_reminder_banner
...
def init_session():
    ...
    if "thread_id" not in st.session_state:
        ...
        logger.info(f"Started new conversation thread: {st.session_state.thread_id}")
...
def render_sidebar(status: dict) -> "st.runtime.uploaded_file_manager.UploadedFile | None":
    st.sidebar.title("⚙️ Assistant")

    name_input = st.sidebar.text_input(
        "Your name",
        value=st.session_state.user_id or "",
        help="Used as your memory key, so preferences and history are remembered next time you use this name.",
    )
    if name_input.strip():
        st.session_state.user_id = name_input.strip().lower().replace(" ", "_")
    ...
    if st.session_state.user_id:
        ...
        st.sidebar.info("Enter your name above to enable long-term memory.")
...
def handle_pdf_upload(uploaded_file, graph, config):
    ...
    graph.update_state(
        config,
        {"pdf_context": text, "user_id": st.session_state.user_id or "default_user"},
    )
...
def main():
    ...
    handle_pdf_upload(uploaded_file, graph, config)

    if st.session_state.user_id:
        render_reminder_banner(get_due_unnotified(st.session_state.user_id))
    ...
    with st.chat_message("assistant"):
        with st.spinner("Thinking..."):
            try:
                result = graph.invoke(
                    {"messages": [user_message], "user_id": st.session_state.user_id or "default_user"},
                    config=config,
                )
